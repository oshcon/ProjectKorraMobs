...
public class LavaBlast {

	public static ConcurrentHashMap<Integer, LavaBlast> instances = new ConcurrentHashMap<Integer, LavaBlast>();
	
	private static double damage = ProjectKorraMobs.plugin.getConfig().getDouble("Abilities.Earth.LavaBlast.Damage");
	...
	public LavaBlast(LivingEntity entity, Location target) {
		this.entity = entity;
		Block targetorigin = MobMethods.getRandomSourceBlock(entity.getLocation(), 3, null, SubElement.Lava);
		...
		origin.getBlock().setType(Material.LAVA);
		origin.getBlock().setData((byte) 5);
